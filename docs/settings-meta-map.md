...
| Global PDF | Chapter flow / image default | Table `almaden_book_settings` + book `post_meta` | `chapter_start_parity`, `chapter_image_default`, `chapter_page_one_align`, `chapter_page_one_vertical` (legacy), `_almaden_book_separate_opening_content`, `_almaden_book_chapter_flow_mode` |
...
| Chapter override | Chapter structural flow / image policy | `post_meta` on chapter | `_start_parity`, `_opening_page_mode`, `_opening_blank_intentional`, `_opening_block_enabled`, `_opening_block_horizontal_align`, `_opening_block_vertical_align`, `_chapter_image_override`, `_parity_image`, `_parity_image_*` |
...
- Chapter image policy: `_chapter_image_override` wins; if absent, `chapter_image_default` applies. Image URL and presentation mode remain chapter-specific.
