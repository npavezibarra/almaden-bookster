...
use AlmadenBookster\Auth\Utilities\Mailer;
use WP_Error;
use WP_User;
...
class VerificationHandler
{
    ...
    public static function confirm_user_email(string $email, string $token)
    {
        if ($email === '' || !is_email($email) || $token === '') {
            return new WP_Error('invalid_token', __('The confirmation link is incomplete.', 'almaden-bookster'));
        }

        $user = get_user_by('email', $email);
        if (!($user instanceof WP_User)) {
            return new WP_Error('invalid_token', __('We could not find that account.', 'almaden-bookster'));
        }
        ...
        if ($stored_hash === '' || $stored_expires < time()) {
            return new WP_Error('token_expired', __('This confirmation token has expired. Please register again or request a new link.', 'almaden-bookster'));
        }

        $provided_hash = hash_hmac('sha256', $token, wp_salt('auth'));
        if (!hash_equals($stored_hash, $provided_hash)) {
            return new WP_Error('invalid_token', __('The confirmation token is invalid.', 'almaden-bookster'));
        }
    ...
    public static function send_confirmation(int $user_id, string $email, string $display_name, string $redirect_to, string $token): bool
    {
        ...
        $sent = false;
        if (class_exists('AlmadenBookster\Auth\Utilities\Mailer')) {
            $sent = (bool) AlmadenBookster\Auth\Utilities\Mailer::send_auth_confirmation($email, $display_name, $verification_url, $token);
        }
