...
class PasswordHandler
{
    ...
    public static function ajax_probe(): void
    {
        ...
        $html = class_exists('\AlmadenBookster\Auth\Utilities\Mailer')
            ? (string) \AlmadenBookster\Auth\Utilities\Mailer::render('password-reset', [
        ...
        if (trim($html) === '') {
            $html = sprintf(
                ...
                esc_html__('Reset your password:', 'almaden-bookster'),
                esc_url($reset_url),
                esc_html($reset_url)
            );
        }

        $subject = __('Reset Password', 'almaden-bookster');
