...
namespace AlmadenBookster\Auth\UI;
...
class Renderer
{
    ...
    public static function get_auth_modal_markup(): string
    {
        ...
        $action_url = AuthUtils::current_page_url();
        $nonce = wp_create_nonce('pl_auth_submit');

        ob_start();
        $template = ALMADEN_BOOKSTER_AUTH_PATH . 'templates/auth-modal.php';
    ...
    public static function get_unverified_popup_markup(): string
    {
        ...
        $data = [
            ...
            'body' => $is_spanish ? 'Para mayor seguridad revisa tu correo y verifica la creación de tu cuenta en Almaden Bookster.' : 'For your security, please check your email and verify the creation of your Almaden Bookster account.',
        ...
        ob_start();
        $template = ALMADEN_BOOKSTER_AUTH_PATH . 'templates/auth/unverified-popup.php';
        if (file_exists($template)) {
            include $template;
        }
        return (string) ob_get_clean();
