...
function almaden_bookster_save_book_ajax() {
	...
	foreach ( $chapters as $chapter ) {
		...
		$subtitle_show          = isset( $chapter['subtitle_show'] ) && in_array( (string) $chapter['subtitle_show'], array( '0', '1' ), true ) ? (string) $chapter['subtitle_show'] : '';
		...
		$subtitle_padding_top   = isset( $chapter['subtitle_padding_top'] ) ? sanitize_text_field( $chapter['subtitle_padding_top'] ) : '';
		$subtitle_padding_bottom = isset( $chapter['subtitle_padding_bottom'] ) ? sanitize_text_field( $chapter['subtitle_padding_bottom'] ) : '';
		$subtitle_padding_left  = isset( $chapter['subtitle_padding_left'] ) ? sanitize_text_field( $chapter['subtitle_padding_left'] ) : '';
		$subtitle_padding_right = isset( $chapter['subtitle_padding_right'] ) ? sanitize_text_field( $chapter['subtitle_padding_right'] ) : '';
		...
		$opening_separate_content = isset( $chapter['opening_separate_content'] ) ? sanitize_text_field( $chapter['opening_separate_content'] ) : '';
			$chapter_image_override = isset( $chapter['chapter_image_override'] ) && in_array( (string) $chapter['chapter_image_override'], array( '0', '1' ), true )
				? (string) $chapter['chapter_image_override']
				: '';
			$chapter_image_enabled = $chapter_image_override;
			$chapter_image_mode    = isset( $chapter['chapter_image_mode'] ) && in_array( $chapter['chapter_image_mode'], array( 'page_blank', 'image_full_page', 'image_inner' ), true )
				? sanitize_text_field( $chapter['chapter_image_mode'] )
				: 'page_blank';
			$chapter_image_url     = isset( $chapter['chapter_image_url'] ) ? esc_url_raw( $chapter['chapter_image_url'] ) : '';
			$chapter_image_inner_width = (string) max( 10, min( 100, (float) ( $chapter['chapter_image_inner_width'] ?? 100 ) ) );
			$chapter_image_inner_header = isset( $chapter['chapter_image_inner_header'] ) && '1' === (string) $chapter['chapter_image_inner_header'] ? '1' : '0';
			$chapter_image_inner_footer = isset( $chapter['chapter_image_inner_footer'] ) && '1' === (string) $chapter['chapter_image_inner_footer'] ? '1' : '0';
		...
		$toc_number_font_style = isset( $chapter['toc_number_font_style'] ) ? sanitize_text_field( $chapter['toc_number_font_style'] ) : '';
		$toc_number_letter_spacing = isset( $chapter['toc_number_letter_spacing'] ) ? sanitize_text_field( $chapter['toc_number_letter_spacing'] ) : '';
		$toc_page_font_family = isset( $chapter['toc_page_font_family'] ) ? sanitize_text_field( $chapter['toc_page_font_family'] ) : '';
		$toc_page_font_size = isset( $chapter['toc_page_font_size'] ) ? sanitize_text_field( $chapter['toc_page_font_size'] ) : '';
		$toc_page_font_weight = isset( $chapter['toc_page_font_weight'] ) ? sanitize_text_field( $chapter['toc_page_font_weight'] ) : '';
		$toc_page_font_style = isset( $chapter['toc_page_font_style'] ) ? sanitize_text_field( $chapter['toc_page_font_style'] ) : '';
		$toc_page_letter_spacing = isset( $chapter['toc_page_letter_spacing'] ) ? sanitize_text_field( $chapter['toc_page_letter_spacing'] ) : '';
		$toc_page_number_offset = isset( $chapter['toc_page_number_offset'] ) ? sanitize_text_field( $chapter['toc_page_number_offset'] ) : '-0.8';
		$toc_page_number_offset = '' !== trim( (string) $toc_page_number_offset ) ? $toc_page_number_offset : '-0.8';
		...
		if ( ! is_wp_error( $post_id ) && $post_id > 0 ) {
			...
			update_post_meta( $post_id, '_subtitle_show', $subtitle_show );
			...
			update_post_meta( $post_id, '_subtitle_padding_top', $subtitle_padding_top );
			update_post_meta( $post_id, '_subtitle_padding_bottom', $subtitle_padding_bottom );
			update_post_meta( $post_id, '_subtitle_padding_left', $subtitle_padding_left );
			update_post_meta( $post_id, '_subtitle_padding_right', $subtitle_padding_right );
			...
			update_post_meta( $post_id, '_chapter_prefix_align', isset( $chapter['chapter_prefix_align'] ) ? sanitize_text_field( $chapter['chapter_prefix_align'] ) : 'center' );
			...
			update_post_meta( $post_id, '_chapter_image_override', $chapter_image_override );
			...
			update_post_meta( $post_id, '_toc_number_font_style', $toc_number_font_style );
			update_post_meta( $post_id, '_toc_number_letter_spacing', $toc_number_letter_spacing );
			update_post_meta( $post_id, '_toc_page_font_family', $toc_page_font_family );
			update_post_meta( $post_id, '_toc_page_font_size', $toc_page_font_size );
			update_post_meta( $post_id, '_toc_page_font_weight', $toc_page_font_weight );
			update_post_meta( $post_id, '_toc_page_font_style', $toc_page_font_style );
			update_post_meta( $post_id, '_toc_page_letter_spacing', $toc_page_letter_spacing );
			update_post_meta( $post_id, '_toc_page_number_offset', $toc_page_number_offset );
			...
			$chapter_response = array(
				...
				'subtitle_margin_top'    => $subtitle_margin_top,
					...
					'subtitle_padding_top'   => $subtitle_padding_top,
					'subtitle_padding_bottom'=> $subtitle_padding_bottom,
					'subtitle_padding_left'  => $subtitle_padding_left,
					'subtitle_padding_right' => $subtitle_padding_right,
				...
				'first_page_footer_type'  => $first_page_footer_type,
					...
					'chapter_prefix_align'   => isset( $chapter['chapter_prefix_align'] ) ? sanitize_text_field( $chapter['chapter_prefix_align'] ) : 'center',
					'opening_separate_content' => $opening_separate_content,
					'chapter_image_enabled'  => $chapter_image_enabled,
					'chapter_image_override' => $chapter_image_override,
				...
				'toc_number_letter_spacing' => $toc_number_letter_spacing,
				'toc_page_font_family' => $toc_page_font_family,
				'toc_page_font_size' => $toc_page_font_size,
				'toc_page_font_weight' => $toc_page_font_weight,
				'toc_page_font_style' => $toc_page_font_style,
				'toc_page_letter_spacing' => $toc_page_letter_spacing,
